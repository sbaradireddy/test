...
Requires the ffmpeg binary (used for audio extraction and keyframe sampling
only - no heavy video ML dependency needed). By default this looks for
"ffmpeg" on PATH; set config.ffmpeg_path to a full executable path if it
isn't on PATH (e.g. on Windows without a PATH edit:
...
class VideoParser(BaseParser):
    ...
    def _ffmpeg_bin(self) -> str:
        return getattr(self.config, "ffmpeg_path", "ffmpeg") if self.config else "ffmpeg"
    ...
    def _extract_audio(self, video_path: str, audio_path: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"ffmpeg binary not found ('{ffmpeg}'). Install ffmpeg and ensure it's on "
                f"PATH, or set config.ffmpeg_path to its full executable path."
            )
            return False
